...
- hip-potential: https://github.com/supermd/hip-potential ，支持 HIP 的 EAM 势函数计算库；
- supermd-hip: https://git.hpcer.dev/HPCer/superMD/supermd-hip ，superMD 核心计算模块的 HIP 实现。
...
dependencies:
  packages:
+   git.hpcer.dev/HPCer/superMD/hip-potential@v0.1.0@hip_pot:
...
### supermd-hip 包
对于 supermd-hip 包，可直接下载源码，我们假设 MSIA-MD 和 supermd-hip 放置的路径如下:
```bash
|-- superMD  # 通用 superMD 源码
|-- supermd-hip  # superMD 核心计算的 HIP 实现 (即 supermd-hip 包)
```
...
cmake -B./cmake-build-hip -S./  \
   ...
   -DMD_HIP_ARCH_SRC_PATH=../supermd-hip \
   -DHIP_HIPCC_FLAGS="-std=c++11 -fgpu-rdc" \
   -DCMAKE_LINKER=../supermd-hip/scripts/hipcc-wrapper.sh \
...
其中在 cmake 配置过程中，`MD_HIP_ARCH_ENABLE_FLAG` 参数表示启用 HIP 环境的支持， `MD_HIP_ARCH_SRC_PATH` 参数指定了 `supermd-hip` 包的源码目录。 
...
5. 链接错误
   ...
   make[2]: *** [frontend/CMakeFiles/supermd.dir/build.make:114: bin/supermd] Error 1
   ```
...
类似于 "hipcc -std=c++11 -lc++ CMakeFiles/supermd.dir/main.cpp.o -o ../bin/supermd  ../src/arch_hip/lib/libmd_arch_hip.a ../lib/libmd.a ../src/arch_hip/lib/libmd_arch_hip.a ..."
