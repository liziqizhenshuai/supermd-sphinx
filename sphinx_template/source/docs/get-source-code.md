...
注：该文档中均使用 `$MD_PATH` 来指代 superMD 源码目录。
...
```bash
git clone https://git.hpcer.dev/HPCer/CrystalMD/CrystalMD.git superMD # https
# or clone from github
git clone https://github.com/supermd/superMD.git
```

或者，如果你配置了ssh key, 也可以使用ssh协议进行clone:
```bash
git clone ssh://git@git.hpcer.dev:2222/HPCer/CrystalMD/CrystalMD.git superMD # ssh
# or clone from github
git clone git@github.com:supermd/superMD.git
```

上述 git clone 命令会创建一个名为 superMD 的目录。
克隆完成后，你可以选择通过调用以下命令来构建特定分支（如版本分支）
```bash
$ cd $MD_PATH # superMD 源码目录
...
superMD 依赖于一些开源库, 如[kiwi](https://git.hpcer.dev/genshen/kiwi),
googletest, fmt, [args](https://github.com/Taywee/args/)等。
可以使用[pkg](https://github.com/genshen/pkg/)依赖管理工具下载依赖包或者直接将对应依赖包导入到 superMD源码`vendor`目录。
...
用户可以从 OSDN 上的 https://osdn.net/pkg/supermd/dependencies 页面获取各个 superMD 版本的依赖包，然后通过 `pkg import` 导入。  
例如，如果需要下载 MISA—MD 主分支（master 分支）对应的依赖包，可以通过链接 https://osdn.net/dl/supermd/misamd-latest-20210413-161112.942981.tar 从[OSDN](https://osdn.net/pkg/supermd/dependencies) 上下载。

### 2.3 直接解压依赖包

直接导入依赖是将已有的依赖压缩包解压解压 superMD 的源码的`vendor`目录。
